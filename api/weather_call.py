...
class geocodingAPI(openAPI):
    ...
    def create_api(self):
        self.api_query = self.base_url + "q=" + self.location + \
            "&appid=" + self.api_key + "&cnt=30&units=metric"

    def load_api(self):
        print(self.api_query)
...
class visualAPI():
    ...
    def run_api(self):
        ...
        CSVText = pandas.DataFrame(csv.DictReader(
            codecs.iterdecode(CSV_Bytes, 'utf-8'), delimiter=','))
