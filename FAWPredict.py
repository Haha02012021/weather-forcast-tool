...
def create_faw_table():
    for i in range(0, NUM_STAGE):
        ...
        faw_table[1].append(
            int((BASE_TEMPURATURE - faw_table[0][i]) / (x * BASE_TEMPURATURE + y)))
...
def get_tempurature(query_time):
    if (query_time > date.today()):
        ...
        try:
            day_index = weather_table[weather_table['datetime']
                                      == query_time].index.values[0]
        except IndexError:
            print("Out of data range")
            exit()
        ...
        if (sys.argv[i] == "--age"):
            ...
            for i in range(len(age_arr)):
                if (max_distribution > age_arr[i][1]):
                    cur_age = age_arr[i][0]
    weather_table_api = visualAPI(
        base_url, api_key, location, start_date, end_date, unit_group, content_type, include)
    weather_table_api.create_api()
    weather_table = weather_table_api.run_api()

    forecast_data = geocodingAPI(
        open_weather_url, api_open_weather_key, location)
